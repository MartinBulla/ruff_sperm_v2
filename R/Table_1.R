  ...
  # exports Table S1a & S1b into ./Outputs/
  # =============================================================
...
# Prepare estimates
  # motility - only one measure per bird (June)
    effects_ = c('Intercept (independent)','Motile count','Satellite relative to independent', 'Faeder relative to independent', 'Faeder relative to satellite')
    lvx = list()
    lvpx =list()
    ...
    # VAP
      ...
      lvx[['VAP']]=data.frame(response='Average path',effect=effects_, estimate=v, lwr=ci[1,], upr=ci[2,])     
    # VSL
      ...
      lvx[['VSL']]=data.frame(response='Straight line',effect=effects_, estimate=v, lwr=ci[1,], upr=ci[2,])  
    # VCL
      ...
      lvx[['VCL']]=data.frame(response='Curvilinear',effect=effects_, estimate=v, lwr=ci[1,], upr=ci[2,])             
  ...
  # morpho - averages
    effects_ = c('Intercept (independent)','Satellite relative to independent', 'Faeder relative to independent','Faeder relative to satellite')
    l = list()
    lcv = list()
...
# Export
  ...
  fwrite(t_w, file = 'Outputs/Table_1a.csv')
  ...
  fwrite(tt_w, file = 'Outputs/Table_1b.csv')
